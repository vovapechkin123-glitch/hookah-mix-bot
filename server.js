...
const ALLOWED_STATUS = new Set(["new","accepted","in_progress","serving","finished"]);
const ALLOWED_RATING = new Set(["bad","ok","perfect"]);
...
app.post("/moment", (req,res)=>{
  try{
    ...
    if(exists){
      return res.status(200).json({ ok:true, moment:exists, message:"Already exists" });
    }

    if(!ALLOWED_STATUS.has(m.status)) m.status = "new";
    moments.push(m);
    writeFileSafe();

    res.status(201).json({ ok:true, moment:m });
  }catch(e){
    res.status(500).json({ ok:false, error:"createMoment error" });
  }
});
